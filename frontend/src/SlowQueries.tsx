...
export default function CollapsibleTable() {
    const history = useHistory()
      const slowQueriesColumns = [
        ...
        { title: 'Avg time (ms)', dataIndex: 'avg_duration', render: (_, item) => <>{item.avg_duration.toFixed(0)}ms</>, sorter: (a, b) => a.avg_duration - b.avg_duration},
        { title: 'Calls / min', dataIndex: 'calls_per_minute', render: (_, item) => <>{item.calls_per_minute.toFixed(3)}</>},
